if (!window._basedflareAuto) {
  class BasedFlareAuto {
    ...
    checkCookie = () => {
      ...
      const powCookie = document.cookie.split("; ").find((row) =>
        row.startsWith("_basedflare_pow=")
      );
      if (powCookie) {
        ...
        if (remainingSecs <= this.cookieMinLife) return this.doBotCheck();
        this.timeout = setTimeout(
          this.checkCookie,
          Math.max(
            5000,
            Math.floor(
              (remainingSecs - this.cookieMinLife + (Math.random() * 300)) *
              1000,
            ),
          ),
        );
    ...
    clearCookiesForDomains = (domain) => {
      ...
      for (let i = 0; i < parts.length - 1; i += 1) {
        ...
        document.cookie =
          `_basedflare_pow=; Max-Age=-9999999; Path=/; Domain=.${subdomain}`;
        document.cookie =
          `_basedflare_captcha=; Max-Age=-9999999; Path=/; Domain=.${subdomain}`;
      }
    };
    messageHandler = (e, json) => {
      ...
      fetch("/.basedflare/bot-check", {
        ...
        body: new URLSearchParams({ "pow_response": `${json.ch}#${answer}` }),
        redirect: "manual",
      }).then((res) => {
    ...
    doProofOfWork = async (json) => {
      ...
      if (mode === "argon2") {
        if (!window.argon2) await this.includeScript(this.scriptSrc);
      }
    ...
    doBotCheck = async () => {
      ...
      try {
        ...
        if (!json || !json.ch) return;
        console.log("Basedflare challenge successfully fetched", json);
        if (json.ca) console.warn("Basedflare auto captcha not yet supported");
        else await this.doProofOfWork(json);
      } catch (e) {
        console.error(e);
        this.fails += 1;
      } finally {
